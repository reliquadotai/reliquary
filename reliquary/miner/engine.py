...
class MiningEngine:
    ...
    def _generate_m_rollouts(self, problem, randomness) -> list[dict]:
        ...
        with torch.no_grad():
            ...
            if BFT_ENABLED:
                # Phase-2 answer generation reuses the protocol sampler so the
                # boxed answer is a valid sample under GRAIL; presence_penalty is
                # dropped — it guards CoT looping, not the short forced answer.
                phase2_kwargs = {
                    "do_sample": True,
                    "temperature": T_PROTO,
                    "top_p": TOP_P_PROTO,
