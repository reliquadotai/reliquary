...
class GraderServer:
    ...
    def _execute_local_sandbox_batch(
        ...
        batch_deadline = time.perf_counter() + request.batch_timeout_s
        results: list[SandboxCaseResult] = []
        result_payload_bytes = 0
        try:
            worker.in_use = True
            for case in request.cases:
                # The first case has the full declared budget. Dispatch
                # overhead must not turn a genuine timeout into an infra fault.
                remaining_batch_s = (
                ...
                if status == "timeout" and remaining_batch_s < request.timeout_s:
                    raise SandboxExecutorError("batch_deadline_exceeded")
