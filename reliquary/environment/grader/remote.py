...
def create_cpu_executor_app(
    ...
    *,
    ...
    async def execute(request: Request) -> Response:
        ...
        def _release_capacity(_future=None) -> None:
            nonlocal api_inflight
            with api_lock:
                api_inflight -= 1
            capacity.release()

        try:
            execution = execution_threads.submit(_execute)
        except Exception:
            _release_capacity()
            raise
        # An ASGI cancellation cannot release a slot still occupied by a
        # running sandbox. A cancelled queued future also completes this hook.
        execution.add_done_callback(_release_capacity)
        try:
            result = await asyncio.wrap_future(execution)
