...
def _completion_keep_list(rollout, prompt_length: int, n_completion: int):
    ...
    """
    # Episode assistant spans are themselves the policy mask; force spans are
    # a legacy single-turn concern and must not be projected onto this compact
    # assistant-only index space.
    if getattr(rollout, "_validated_assistant_spans", None) is not None:
        return None
...
def _completion_token_logprobs(rollout) -> list:
    ...
    if positions:
        if tokens and len(old) == len(tokens):
            return [old[position] for position in positions]
        return old
    if tokens and len(old) == len(tokens):
...
def _policy_token_positions(rollout) -> list[int]:
    """Absolute token positions generated by the policy.

    Episode spans are accepted only from the validator-derived private field.
    A wire episode without replayed spans fails closed with no trainable tokens.
    Legacy rollouts retain the contiguous prompt-to-end completion range.
    """

    commit = getattr(rollout, "commit", None) or {}
    tokens = list(commit.get("tokens", []) or [])
    meta = commit.get("rollout", {}) or {}
    episode = meta.get("episode")
    spans = getattr(rollout, "_validated_assistant_spans", None)
    if episode is not None:
        ...
        return positions
    prompt_length = int(meta.get("prompt_length", 0) or 0)
    return list(range(prompt_length, len(tokens)))
...
def _rollout_loss(
    ...
    *,
    ...
    policy_positions = _policy_token_positions(rollout)
    if prompt_length <= 0 or not old_logprobs_list or not policy_positions:
    ...
    position_index = torch.tensor(
        [position - 1 for position in policy_positions],
        device=device,
        dtype=torch.long,
    )
    new_logprobs_c = new_logprobs.index_select(0, position_index)
    ...
    if need_ref_forward:
        ...
        ref_logprobs_c = ref_logprobs.index_select(0, position_index)
    ...
    if need_pi_forward:
        if share:
            ...
            behavior_logprobs_c = behavior_logprobs.index_select(
                0, position_index
            )
...
def _batched_completion_logprobs(
    model,
    input_ids,
    attention_mask,
    prompt_lengths,
    ...
    for i, (p, L) in enumerate(zip(prompt_lengths, lengths)):
        positions = (
            ...
            else list(policy_positions[i])
        )
        seg.append(len(positions))
        for token_position in positions:
            b_rows.append(i)
            p_rows.append(int(token_position) - 1)
...
def _microbatch_grad(
    ...
    *,
    ...
    plens, lens, olds, advs, scales, keeps, volds, positions = (
        [], [], [], [], [], [], [], []
    )
    for j, item in enumerate(batch):
        tokens, p, old, adv, scale, keep, vold = item[:7]
        L = len(tokens)
        ...
        volds.append(vold)
        positions.append(
            list(item[7]) if len(item) > 7 else list(range(p, L))
        )
    ...
    with torch.autocast(device_type=device.type, dtype=torch.bfloat16,
        ...
        new_lp, seg = _batched_completion_logprobs(
            model, input_ids, attn, plens, lens, positions
        )
        with torch.no_grad():
            ref_lp = None
            behavior_lp = None
            if need_ref_forward:
                ref_lp, _ = _batched_completion_logprobs(
                    ref_model, input_ids, attn, plens, lens, positions
                )
            if need_pi_forward:
                if share:
                    behavior_lp = ref_lp
                    ...
                    behavior_lp, behavior_seg = _batched_completion_logprobs(
                        behavior_model, input_ids, attn, plens, lens, positions
                    )
                    if behavior_seg != seg:
                        raise ValueError(
