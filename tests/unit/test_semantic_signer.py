...
from reliquary.signer.journal import (
    JournalConflict, JournalReplay, JournalUncertain, SignerJournal,
)
...
def test_restart_retries_only_unfinished_checkpoint_signatures(tmp_path):
    path = str(tmp_path / "journal" / "signer.sqlite3")
    checkpoint = _checkpoint_request()
    weight = _weight_request()
    journal = SignerJournal(path)
    for request, kind, cursor, value in (
        (checkpoint, "checkpoint-sign", "checkpoint_n", 1),
        (weight, "set-weights", "weight_epoch", 100),
    ):
        journal.reserve(
            operation_id=request["operation_id"], kind=kind,
            payload_digest=request["operation_id"],
            cursor_name=cursor, cursor_value=value,
        )
    journal.close()
    journal = SignerJournal(path)
    backend = FakeBackend()
    app = create_signer_app(
        policy=SignerPolicy(network="finney", netuid=81, repo_id=REPO),
        backend=backend, journal=journal,
    )
    try:
        with TestClient(app) as client:
            assert client.post("/v1/checkpoints/sign", json=checkpoint).status_code == 200
            assert client.post("/v1/checkpoints/sign", json=checkpoint).json()["cached"]
            assert client.post("/v1/weights/set", json=weight).status_code == 409
        assert backend.checkpoint_calls == 1
        assert backend.weight_calls == 0
        with pytest.raises(JournalConflict):
            journal.reserve(
                operation_id=weight["operation_id"], kind="set-weights",
                payload_digest=weight["operation_id"],
            )
    finally:
        journal.close()
