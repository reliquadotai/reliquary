...
def test_episode_metadata_cannot_select_single_turn_admission_mode(monkeypatch):
    from dataclasses import replace

    original = admission.score_and_finalize_submission

    def wrong_environment(parsed, materials, context, deadline):
        return original(parsed, materials, replace(context, environment="openmathinstruct"), deadline)

    monkeypatch.setattr(__import__(__name__, fromlist=["score_and_finalize_submission"]), "score_and_finalize_submission", wrong_environment)
    request, _, result = _score(monkeypatch)
    assert result.reject_reason is RejectReason.BAD_SCHEMA
    assert result.reject_stage == "environment_interaction"
    assert all(getattr(rollout, "_validated_assistant_spans", None) is None for rollout in request.rollouts)
    assert admission.submission_interaction_matches(request, "reliquary_retrieval_tools_v1")
    for rollout in request.rollouts:
        rollout.commit["rollout"].pop("episode")
    assert not admission.submission_interaction_matches(request, "reliquary_retrieval_tools_v1")
    assert admission.submission_interaction_matches(request, "openmathinstruct")
