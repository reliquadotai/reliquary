...
@pytest.mark.parametrize("used", [None, True, -1, "200", 2.5])
def test_unknown_or_invalid_usage_fails_closed(used):
    api = _Api()
    api.dataset_info = lambda repo_id: SimpleNamespace(used_storage=used)
    guard = HfStorageGuard(policy=HfStoragePolicy(freeze_bytes=1000), api=api)
    with pytest.raises(RuntimeError, match="usage is unavailable or invalid"):
        guard.assert_upload_allowed(repo_id="ReliquaryForge/model", upload_bytes=1)
