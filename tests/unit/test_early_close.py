...
def test_conservative_valuation_cannot_break_dominance(monkeypatch):
    """Early close seals when B_BATCH prompts sit at the exact value ceiling.

    Conservative truncation valuation only ever lowers a group's value (it is a
    minimum over interpretations), so it can never push a candidate ABOVE the
    ceiling — the dominance argument survives. It does make the bar stricter: a
    group carrying truncations no longer reaches the ceiling, so early close
    fires only on truncation-free candidates.
    """
    ...
    ceiling = max_difficulty_value(8)
    at_peak = [1.0] * 2 + [0.0] * 6                 # k=2 is the value peak
    assert difficulty_score(at_peak).value == ceiling

    for truncated in range(1, 5):
        value = conservative_difficulty_score(
            at_peak, truncated_count=truncated
        ).value
        assert value <= ceiling                      # never above
        assert value != ceiling                      # and never AT it either
