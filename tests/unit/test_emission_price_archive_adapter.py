...
validator did not instrument the window, while ``collect_ready_block: null``
...
def _instrumented(**overrides) -> dict:
    record = {
        ...
        "window_open_block": 1000,
        "window_close_block": 1100,
        "collect_ready_block": 1010,
        "training_blocks": 40.0,
        "validation_blocks": 25.0,
    }
...
def test_an_instrumented_window_reads_back_whole():
    ...
    assert outcome.elapsed_blocks == 100
    assert outcome.filled
    assert outcome.ratio == 0.25   # 10 blocks of collection against 40
...
def test_an_explicit_null_ready_block_is_a_shortage_not_silence():
    """The window was instrumented and it did not fill. That is real news."""
    outcome = outcome_from_archive(_instrumented(collect_ready_block=None))
