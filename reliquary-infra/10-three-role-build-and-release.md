...
After signer restart, an unfinished `checkpoint-sign` reservation may retry
the exact claim, because producing a signature has no chain side effect.
Completed operations retain their cached bytes. Pending or uncertain chain
writes remain non-replayable; their outcome needs explicit reconciliation.
...
Build on a trusted Git checkout after committing the release revision. Both
builders reject staged and unstaged source changes and send a Git archive of
that exact revision to Docker, excluding local ignored/untracked material.
Evidence checksums use relative names so the bundle remains verifiable after
transfer:
...
Before an authoritative control deployment, implement and qualify the remote
GPU proof boundary and the wallet-free validator Compose configuration. The
existing trainer Compose still requires a local wallet mount; configuring a
signer URL alone does not remove that mount or deploy a validator on ctrl-01.
Retain the current detached trainer and proof scheduler.

The physical and production qualification gates are:
