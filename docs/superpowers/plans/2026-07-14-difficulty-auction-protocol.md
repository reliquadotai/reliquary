...
- **Arming is out of scope.** Production emission keeps using `select_batch_and_distribute` until the free-negative measurement clears (spec §9). This plan changes *when we prove* and *when we seal*, not *who gets paid*.
