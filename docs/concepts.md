...
At every nonempty V6 journal-window boundary, the trainer flushes accumulated updates, saves the model, pushes it to HF Hub, and signs `ed25519(checkpoint_n || revision)`. The journal reserves `CHECKPOINT_PUBLISH_INTERVAL_WINDOWS = 16` slots; with the default 16 picks this is 16 payloads, while a smaller qualified pick target fills the unused slots with tombstones and still publishes after its real payloads. If the PPO ratio gate detects behavior-policy drift sooner, the rejected step is excluded and the previously accepted in-memory updates are published immediately. The next window stays closed until the signed successor covering the consumed journal cursor is installed in `/checkpoint` and the proof plane. The window dataset is archived to R2, including quarantine metadata when present.

**9. State → READY → OPEN.**
After the journal-consumption and successor-adoption barrier clears, the next window opens. Winning prompts enter one-shot cooldown. Once per subnet epoch the validator calls `set_weights` on-chain with the current EMA snapshot.
...
### Publish at nonempty fill-window boundaries — HF cannot keep up with per-step pushes

The base model is Qwen3-4B-Base, used with raw prompt text rather than a chat template. Pushing a new safetensors snapshot to HF Hub after every payload is infeasible due to Git LFS latency and HF rate limits. The journal keeps 16 slots per fill window (the legacy constant name remains `CHECKPOINT_PUBLISH_INTERVAL_WINDOWS`), and the trainer publishes once at each nonempty boundary after flushing its successful updates. Quarantined or ratio-rejected payloads do not increment the successful-step counter, but a real payload still makes the boundary publish a covering checkpoint so the next window cannot open on stale weights. If `policy_ratio_drift` rejects a later step, that rejected step never reaches the optimizer and the trainer publishes the preceding safe updates, refreshes the serving behavior policy, and retries a failed upload without training again. Between publishes, miners stay on the last-published revision — the hash gate keeps them there. `checkpoint_n` only increments on a successful publish, so the gate remains stable across the publish gap.
