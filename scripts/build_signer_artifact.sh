...
for command_name in git docker gzip jq sha256sum; do
...
if git -C "${REPO_DIR}" rev-parse HEAD >/dev/null 2>&1; then
  ...
  if ! git -C "${REPO_DIR}" diff --quiet HEAD -- \
    ...
    reliquary/__init__.py \
    reliquary/signer; then
    echo "refusing to build with uncommitted signer source changes" >&2
    exit 2
  fi
else
  echo "a Git checkout is required to bind the build context to its revision" >&2
  exit 2
fi
...
git -C "${REPO_DIR}" archive "${REVISION}" | docker build \
  --progress plain \
  --platform linux/amd64 \
  --network host \
  --build-arg "RELIQUARY_BUILD_REVISION=${REVISION}" \
  --tag "${IMAGE_TAG}" \
  --file docker/Dockerfile.signer \
  -
...
(
  cd "${OUTPUT_DIR}"
  sha256sum manifest.json image-inspect.json image-history.txt \
    python-packages.txt protocol-version.txt runtime-import.txt > evidence.sha256
)
